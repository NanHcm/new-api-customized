...
// PlanRecommendedModels returns known-good model IDs for subscription plans
// that do NOT expose a public /models endpoint (e.g. VolcEngine Agent Plan).
// When a user clicks "Fetch Models" on such a plan, fetchChannelUpstreamModelIDs
// falls back to this list so they can still populate their channel without
// hand-typing every model ID.
//
// The list is intentionally conservative: it only covers the well-known plan
// roots. Roots not listed here keep the previous behavior (return the upstream
// error verbatim). Update this map when a vendor publishes a new model on one
// of these plans.
var PlanRecommendedModels = map[string][]string{
	// VolcEngine Agent Plan (no /models endpoint as of 2026-07).
	// Source: probing https://ark.cn-beijing.volces.com/api/plan/v3/models
	// returns 404 for every code path.
